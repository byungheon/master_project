...
%     local.dynamics    = @dynamics_NOGP_learning;
%     local.OPTIONS     = odeset('RelTol', 1e-7, 'AbsTol', 1e-7);
%     local.ctrlfcn     = str2func('zoh_local');   
%     local.par.dt      = dt; local.par.delay = 0; local.par.tau = dt;
%     local.u0          = cell(1,length(jointi)-1);
%     local.delta       = zeros(1,length(difi));
%     for i = 1:size(targets_temp,1)
%         local.tau = inputs_temp(i,end-Du+1:end)';
%         
%         for ii = 1:length(jointi)-1, local.u0{ii} = @(t)local.ctrlfcn(local.tau(ii,:),t,local.par); end
