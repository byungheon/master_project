...
% 1. Initialization
clear all; close all;
settings_NOGP;                 % load scenario-specific settings
basename = 'NOGP_'; % filename used for saving data
% 2. Initial J random rollouts
for jj = 1:J
  [xx, yy, realCost{jj}, latent{jj}] = ...
    rollout_GP(gaussian(mu0, S0), struct('maxU',policy.maxU), H, plant, cost, dynmodel);
...
for j = 1:N
  tic
  trainDynModel_NOGP;   % train (GP) dynamics model
  learnPolicy;     % learn policy
  applyController_NOGP; % apply controller to system
