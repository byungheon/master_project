...
%  5~6     dtheta        angular velocity of a pendulum
%  7~8     theta         angle of a pendulum
%  9     sin(q1)       complex representation ...
%  10     cos(q1)       ... of theta
%  11     sin(q2)       complex representation ...
%  12     cos(q2)       ... of theta
%  13     sin(theta1)       complex representation ...
%  14     cos(theta1)       ... of theta
%  15     sin(theta2)       complex representation ...
%  16     cos(theta2)       ... of theta
%  17~18  u             joint torque that can be applied at each joint
...
odei = [1 2 7 8 3 4 5 6];
augi = [];
dyno = [1 2 3 4 5 6 7 8];
angi = [1 2 7 8];
dyni = [3 4 5 6 9 10 11 12 13 14 15 16];
dyni_p = [1 2 3 4 5 6 9 10 11 12 13 14 15 16];
poli = [3 4 5 6 9 10 11 12 13 14 15 16];
difi = [1 2 3 4 5 6 7 8];
jointi = [1 2];
...
s = (0.01^2) * ones(1,8); % initial state variances
S0 = diag(s);             % initial state covariance matrix
mu0 = zeros(8,1);         % initial state mean
...
plant.dynamics = @dynamics_kpdsu;           % handle to dynamics ODE function
...
policy.fcn = @(policy,m,s)conCat(@congp,@gSat,policy,m,s); % controller 
                                                           % representation
policy.maxU = [130 90];                                    % max. amplitude of 
                                                           % control
...
cost.fcn = @loss_kpdsu;                           % handle to cost function
cost.gamma = 1;                                   % discount factor
cost.p = [0.5 0.4 0.8 0.8];                               % lenghts of the links and length of pendulum
cost.width = 0.5;                                 % cost function width
cost.expl = 0;                                    % exploration parameter
cost.angle = plant.angi;                          % angle variables in cost
cost.target = zeros(8,1);
cost.target(7)  = pi;
cost.target(8)  = pi;
